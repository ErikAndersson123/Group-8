...
public class DataLogic extends UnicastRemoteObject implements Subject {
    ...
    public Chatroom[] userChatrooms(User user) throws RemoteException {
        ...
        for (Chatroom chatroom : chatrooms) {
            for (User u : chatroom.getChatroomUsers()) {
                if (u.getUserID() == user.getUserID()) {
                    //c[i] = chatroom;
                    //System.out.println(c[i].getRoomID());
                    i++;
                }
            }
        }
        int j = 0;
        Chatroom[] c = new Chatroom[i];
        for (Chatroom chatroom : chatrooms) {
            for (User u : chatroom.getChatroomUsers()) {
                if (u.getUserID() == user.getUserID()) {
                    c[j] = chatroom;
                    System.out.println(c[j].getRoomID());
                    j++;
                }
            }
        }
